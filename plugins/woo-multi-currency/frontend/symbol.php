...
class WOOMULTI_CURRENCY_F_Frontend_Symbol {
	...
	public function __construct() {
		...
		if ( $this->settings->get_enable() ) {
			 ...
			 * Format price
			 */
			add_filter( 'wc_price_args', array( $this, 'wc_price_args' ) );
			/**
			 * Symbol position
			 */
	 ...
	 * @return int
	 */
	protected function get_price_decimals() {
		$selected_currencies = $this->settings->get_list_currencies();
		try {
			return absint( $selected_currencies[ $this->settings->get_current_currency() ]['decimals'] );
		}
		catch ( Exception $e ) {
			return absint( $selected_currencies[ $this->settings->get_default_currency() ]['decimals'] );
		}
	}
